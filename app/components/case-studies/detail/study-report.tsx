    ...
    <article className="min-w-0 overflow-hidden rounded-2xl border border-[#dce5ed] bg-white text-[#18344e] shadow-sm">
      ...
      {study.sections.map((section, index) => {
        ...
        return <section key={section.title} className="border-t border-[#dce5ed] p-5 sm:p-10 lg:p-12">
          ...
          {section.tables?.map((table, ti) => <div key={ti} className="mt-6 overflow-x-auto">{table.caption && <p className="mb-2 text-sm font-semibold text-[#173e58]">{table.caption}</p>}<table className="w-full min-w-120 border-collapse text-left text-sm"><thead className="bg-[#1b3b5d] text-white"><tr>{table.headers.map(h => <th key={h} scope="col" className="p-3">{h}</th>)}</tr></thead><tbody>{table.rows.map((row, i) => <tr key={i} className="border-b border-[#dce5ed] odd:bg-[#f5f9fb]">{row.map((cell, j) => j === 0 ? <th key={j} scope="row" className="p-3 align-top font-semibold">{cell}</th> : <td key={j} className="p-3 align-top leading-6">{cell}</td>)}</tr>)}</tbody></table></div>)}
          {section.quotes && <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{section.quotes.map(q => <blockquote key={q.text} className="rounded-lg border border-[#dce5ed] bg-[#f5f9fb] p-4 text-sm italic leading-6 text-slate-700"><p>&ldquo;{q.text}&rdquo;</p><cite className="mt-3 block text-xs font-semibold not-italic text-[#173e58]">— {q.source}</cite></blockquote>)}</div>}
          ...
          {visual?.insight && <aside className="mt-6 border-l-4 border-[#1bb5a5] bg-[#edf7f9] px-5 py-4 text-sm leading-7"><strong className="mr-2 text-xs tracking-wider">KEY INSIGHT</strong>{visual.insight}</aside>}
        </section>;
      })}
