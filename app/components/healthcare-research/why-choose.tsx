        ...
        return (
          <div key={item.title} className="border-b border-black">
            ...
            >
              <div>
                <h3
                  className={`text-sm font-semibold lg:text-lg ${isOpen ? "text-[#16af9f]" : "text-[#384250]"
                    }`}
                >
                  {item.title}
                </h3>
              ...
              <span className="ml-4 shrink-0">
                <ChevronDown
                  ...
                  className={`transition-transform duration-500 motion-reduce:transition-none ${
                    isOpen ? "rotate-180 text-white" : "text-[#4b5563]"
                  }`}
                />
              </span>
            </button>
...
export default function WhyChooseHealth({ data }: WhyChooseHealthProps) {
  const sectionsData = useMemo(() => data?.sections || [], [data?.sections]);
  ...
  if (!data || sectionsData.length === 0) return null;
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 py-10 lg:px-0 lg:py-16">
        <div className="space-y-24 lg:space-y-20">
          {sectionsData.map((section, sectionIndex) => {
            ...
            const activeImage =
              section.accordionData?.[activeIndex]?.image || section.image;
                    ...
                    {section.useAnimatedFigure ? (
                      ...
                      activeImage && (
                        <Image
                          ...
                          height={520}
                          unoptimized
                          className="relative z-10 h-auto w-full object-contain p-8 transition-all duration-300"
                        />
                      )
                    )}
                  </div>
